...
def check_nums(list_of_num):
    ...
    while index <= len(list_of_num):
        if list_of_num[index] == 7:
            break
        output_list.append(list_of_num[index])
        index=index+1
    return output_list
...
def sublist(list_str):
    ...
    while index < len(list_str):
        if list_str[index] == "STOP":
            break

        new_list_str.append(list_str[index])
        index = index + 1
...
def stop_at_z(list_of_let):
    new_list=[]
    index=0
    while index < len(list_of_let):
        if list_of_let[index] == "z":
            break
        new_list.append(list_of_let[index])
        index=index+1
    return new_list
